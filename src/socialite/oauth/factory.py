...
from socialite.oauth.amazon import AmazonAPI
...
class ProviderFactory:
    ...
    _oauth2_providers = ["amazon", "google", "facebook", "linkedin", "github"]
    ...
    def make(
        ...
        if cls.name in cls._oauth1_providers:
            ...
            raise ProviderAPIDoesNotExistsError(
                "The provider '{name}' doesn't supported".format(name=name)
            )
        ...
        if cls._provider_type == "oauth1":
            cls.__socialite_api__ = cls.providers.get(cls.name)(
                cls.resource_owner_key, cls.resource_owner_secret
            )
        elif cls._provider_type == "oauth2":
            cls.__socialite_api__ = cls.providers.get(cls.name)(
                {"token_type": token_type, "access_token": cls.access_token}
            )
